...
Puppet::Type.type(:docker_compose).provide(:ruby) do
  ...
  def create
    ...
    args = ['compose', compose_files, '-p', name, 'up', '-d', '--remove-orphans'].insert(4, resource[:options]).insert(5, resource[:up_args]).compact
    docker(args)
    ...
    args = ['compose', compose_files, '-p', name, 'scale'].insert(4, resource[:options]).compact + instructions
    docker(args)
  end

  def destroy
    Puppet.info("Removing all containers for compose project #{name}")
    kill_args = ['compose', compose_files, '-p', name, 'kill'].insert(4, resource[:options]).compact
    docker(kill_args)
    rm_args = ['compose', compose_files, '-p', name, 'rm', '--force', '-v'].insert(4, resource[:options]).compact
    docker(rm_args)
  end
  ...
  def restart
    ...
    kill_args = ['compose', compose_files, '-p', name, 'kill'].insert(4, resource[:options]).compact
    docker(kill_args)
    build_args = ['compose', compose_files, '-p', name, 'build'].insert(4, resource[:options]).compact
    docker(build_args)
    create
  end
